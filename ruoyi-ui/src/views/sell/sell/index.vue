<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="big" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="商品名:" prop="sellName">
        <el-autocomplete
          class="inline-input"
          v-model="suggestion"
    ...
    <el-table v-loading="loading" :data="sellList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="到达时间" align="center" prop="arrivalTime" />
      <el-table-column label="售卖规格" align="center" prop="tp" />
      <el-table-column label="检验时间" align="center" prop="checkTime" />
      <el-table-column label="检验项目" align="center" prop="checkName" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="到达时间" prop="arrivalTime">
          <el-date-picker type="date" v-model="form.arrivalTime"  placeholder="选择一个日期" value-format="yyyy-MM-dd"> </el-date-picker>
        </el-form-item>
        <el-form-item label="售卖数量" prop="amount">
          <el-input placeholder="请输入售卖数量" v-model="form.amount" clearable>
          </el-input>
        </el-form-item>
        ...
        <el-form-item label="检验项目" prop="checkName">
          <el-input placeholder="请输入检验项目" v-model="form.checkName" clearable>
          </el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    querySearch(queryString, cb) {
      ...
      let results = queryString ? sells.filter(this.createFilter(queryString)) : sells;
      // 调用 callback 返回建议列表的数据
      cb(results);
    },
    createFilter(queryString) {
      return (sell) => {
        return (sell.value.toLowerCase().indexOf(queryString.toLowerCase()) > -1);
      };
    },
    handleSelect(item) {
    ...
    handleQuery(isSilent = false) {
      ...
      querySellByPid(pidToQuery).then(response => {
        ...
        // 1. 初始化校验状态
        this.sellList = rawList.map(item => {
          if (!item.params) {
            item.params = {};
          }
        ...
        const hasProcessingItem = this.sellList.some(item => item.status === 0);
    ...
    verifyListItems() {
      this.sellList.forEach(item => {
        ...
        if (item.status === 1 && item.params.verifyStatus === 'loading') {
          ...
          // 调用后端 getSell 接口 (对应后端的 getInfo)
          getSell(item.id).then(res => {
